...
const WorkoutChart = (props) => {
  ...
  const filterWorkouts = (filter) => {
    ...
    if (filter === "Daily") {
      const range = 7;
      const grouped = props.workouts.reduce((prev, curr) => {
        const day = dayjs(curr.datecompleted).hour(0).format("YYYY-MM-DD");
      ...
      const range = 8;
      // total workouts by week
      const grouped = props.workouts.reduce((prev, curr) => {
        const week = dayjs(curr.datecompleted).weekday(0).format("YYYY-MM-DD");
        if (prev[week]) prev[week] += 1;
        else prev[week] = 1;
      ...
      const grouped = props.workouts.reduce((prev, curr) => {
        const month = dayjs(curr.datecompleted).format("MMM-YY");
